...
from utils import TimeBlock #Gated-STGCN
from utils import TimeBlock_padding

from utils import ST_BLOCK_5 #GRCN
from utils import ST_BLOCK_6 #OTSGGCN
from utils import ST_BLOCK_7 #ADGCN
from utils import ST_BLOCK_8 #ADGCN
from utils import multi_gcn #gwnet
from utils import GCNPool #H_GCN
from utils import Transmit
from utils import gate
# TGCN
from utils import TGCNGraphConvolution, TGCNCell
...
# ADGCN
from utils import SpatialAttention
from utils import SpatialAttention_fast
...
class ASTGCN_Recent(nn.Module):
    def __init__(self, device, num_nodes, dropout=0.3, supports=None, seq_length=12,
                 in_dim=1, out_dim=12, residual_channels=32, dilation_channels=64,
                 skip_channels=256, end_channels=512, kernel_size=2, K=3, Kt=3):
        super(ASTGCN_Recent, self).__init__()
        # 切比雪夫阶数
        # K = 1
        self.block1 = ST_BLOCK_0(in_dim, dilation_channels, num_nodes, seq_length, K, Kt)
        self.block2 = ST_BLOCK_0(dilation_channels, dilation_channels, num_nodes, seq_length, K, Kt)
        '''out_channels对应了模型输出的时间步长度'''
        # self.final_conv = Conv2d(in_channels=length, out_channels=12, kernel_size=(1, dilation_channels),
        #                          padding=(0,0), stride=(1,1), bias=True)
        # self.final_conv = Conv2d(in_channels=length, out_channels=3, kernel_size=(1, dilation_channels),
        #                          padding=(0, 0), stride=(1, 1), bias=True)
        # '''原作者将输入的时间轴作为输入通道,N和C作为H和W'''
        self.final_conv = Conv2d(in_channels=seq_length, out_channels=out_dim, kernel_size=(1, dilation_channels),
                                 padding=(0, 0), stride=(1, 1), bias=True)
        # self.final_conv = Conv2d(in_channels=dilation_channels, out_channels=1, kernel_size=(1, seq_length),
        #                          padding=(0, 0), stride=(1, 1), bias=True)
...
class TGCN(nn.Module):
    def __init__(self,device, num_nodes, dropout=0.3, supports=None,length=12,
        ...
        self.device = device
        self._input_dim = len(supports)
        self._hidden_dim = 10
        self.pre_len = 1
        # self.register_buffer("adj", torch.FloatTensor(torch.stack(supports)))
        # self.register_buffer("adj", torch.stack(supports))
        self.adj = torch.stack(supports)
        self.tgcn_cell = TGCNCell(self.adj, self._input_dim, self._hidden_dim, self.device)
        # 全连接
        self.regressor = nn.Linear(self._hidden_dim, self.pre_len)
    ...
    def forward(self, inputs):
        ...
        hidden_state = torch.zeros(batch_size, num_nodes * self._hidden_dim).type_as(inputs).to(self.device)
        output = None
        for i in range(seq_len):
            output, hidden_state = self.tgcn_cell(inputs[:, i, :], hidden_state)
            output = output.reshape((batch_size, num_nodes, self._hidden_dim))
        # output shape = [batch_size, num_nodes, hidden_len]
...
params:
    ...
    in_dim:输入维度
    out_dim:输出维度
    residual_channels:还不知道, 残差通道?
    dilation_channels:还不知道, 膨胀通道?
    skip_channels:同上
    end_channels:同上
"""
class Gated_STGCN(nn.Module):
    ...
    def __init__(self, device, num_nodes, dropout=0.3, supports=None, seq_length=12,
                 in_dim=1, out_dim=3, residual_channels=32, dilation_channels=32,
                 skip_channels=256, end_channels=512, kernel_size=2, K=3, Kt=3):
        super(Gated_STGCN, self).__init__()
        # 序列长度
        # tem_size = seq_length
        #
        self.block1 = ST_BLOCK_4(in_dim, dilation_channels, num_nodes, seq_length, K, Kt)
        self.block2 = ST_BLOCK_4(dilation_channels, dilation_channels, num_nodes, seq_length, K, Kt)

        # 这里的12也许是对应数据集的预测长度
        # self.conv1=Conv2d(dilation_channels,12,kernel_size=(1, tem_size),padding=(0,0),
                          # stride=(1,1), bias=True)
        # PEMS数据集改为3/6/9
        # self.conv1 = Conv2d(dilation_channels, 9, kernel_size=(1, seq_length), padding=(0, 0), stride=(1, 1),
        #                     bias=True)

        out_channels = 128
        # self.last_temporal = TimeBlock(in_channels=128, out_channels=out_channels)
        # '''改成padding，不改变时间序列长度'''
        self.last_temporal = TimeBlock_padding(in_channels=128, out_channels=out_channels)

        # 用于最后的全连接, 输出的维度为target_len
        # self.fully = nn.Linear((seq_length - 2 * 5) * out_channels, out_dim)
        # '''加入在时序卷积过程中一直padding呢'''
        self.fully = nn.Linear(seq_length * out_channels, out_dim)
...
# attention dynamic gcn
class ADGCN(nn.Module):
    def __init__(self, device, num_nodes, dropout=0.3, supports=None, seq_length=12,
                 in_dim=1, out_dim=12, residual_channels=32, dilation_channels=32,
                 skip_channels=256, end_channels=512, kernel_size=2, K=3, Kt=3, adj_list=None):

        super(ADGCN, self).__init__()

        # 邻接表
        self.adj_list = adj_list
        # 建立道路id与矩阵下标的映射, 矩阵下标与道路id的映射
        self.id_idx = dict()
        self.idx_id = dict()
        for i, road in enumerate(self.adj_list.keys()):
            self.id_idx[road] = i
            self.idx_id[i] = road

        # 设置TPL卷积卷积核大小
        # self.spatial_attention = SpatialAttention(adj_list=adj_list)
        self.spatial_attention = SpatialAttention_fast()
        # 原始邻接矩阵
        self.adj = supports[0]
        # 与原始邻接矩阵结合
        self.help_1 = nn.Parameter(torch.zeros(num_nodes, num_nodes))
        self.help_2 = nn.Parameter(torch.zeros(num_nodes, num_nodes))
        self.help_b = nn.Parameter(torch.zeros(num_nodes, num_nodes))

        '''与原始邻接矩阵结合,使用单个参数加权'''
        self.help_3 = nn.Parameter(torch.randn(size=(1, 1)))
        self.help_4 = nn.Parameter(torch.randn(size=(1, 1)))
        ...
        # 使用inception结构的卷积
        self.block1 = ST_BLOCK_8(in_dim, dilation_channels, num_nodes, tem_size, K, Kt)
        self.block2 = ST_BLOCK_8(dilation_channels, dilation_channels, num_nodes, tem_size, K, Kt)
        ...
        self.lp_wrapper = self.lp(self.spatial_attention.forward)

    '''tpl的维度为[batch_size, timestamp]'''
    def forward(self, input_x, input_tpl):
        # input_tpl 做空间注意力
        w = self.spatial_attention(input_tpl)
        # w = self.lp_wrapper(input_tpl)
        # 输出性能检测结果
        # self.lp.print_stats()
        # w 与距离邻接矩阵加权1
        # w = torch.matmul(self.help_1, w) + torch.mul(self.help_2, self.adj) + self.help_b
        w = self.help_3 * w + self.help_4 * self.adj + self.help_b
        # w作为图卷积的邻接矩阵
        x = input_x.permute(0, 2, 3, 1)

        '''这里做通道注意力的数据应该是[B, N, N, T]'''
        # 通过邻接表还原流量转移矩阵
        # TPML = self.restore(input_tpl)
        # ca = self.channel_attention(TPML)
        # ca = ca.permute(0, 2, 1, 3)
        # x = torch.mul(ca, x)
    ...
    通过邻接表与tpl还原流量转移矩阵序列
    """
    def restore(self, tpl):
        # tpl的维度为[batch_size, timestamp, N, C]

        batch_size, time_len, N, C = tpl.shape

        tpml = torch.empty(size=(batch_size, time_len, N, N))

        for b in range(batch_size):
            for t in range(time_len):
                for n in range(N):
                    for i, neighbor in enumerate(self.adj_list[self.idx_id[n]]):
                        tpml[b, t, n, self.id_idx[neighbor]] = int(tpl[b, t, n, i])
        return tpml.cuda()
